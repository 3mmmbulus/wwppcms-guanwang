...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      try {
        const result = await pb.collection('tasks').getList<Task>(1, 500, {
        ...
        // Update stats
        const todoCount = result.items?.filter(t => t.status === 'todo').length || 0;
        const inProgressCount = result.items?.filter(t => t.status === 'in_progress').length || 0;
        const completedCount = result.items?.filter(t => t.status === 'completed').length || 0;
        ...
        console.warn('Failed to fetch tasks (skipping, will show empty):', error);
        setTasks([]);
        setStats([
          { title: '待办任务', value: '0', icon: Bookmark01Icon, color: 'text-neutral-600 dark:text-neutral-400', bgColor: 'bg-neutral-50 dark:bg-neutral-900/50' },
          { title: '进行中', value: '0', icon: HourglassIcon, color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-50 dark:bg-blue-950/50' },
